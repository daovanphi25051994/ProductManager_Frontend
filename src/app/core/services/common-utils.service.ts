...
//     return (
//       CommonUtils.tctCompareDates(startDate, check) <= 0 &&
//       CommonUtils.tctCompareDates(check, endDate) <= 0
//     );
//   }
...
//         CommonUtils.tctCompareDates(startDate2, endDate1) <= 0
//       );
//     } else {
//       return (
//         (CommonUtils.isNullOrEmpty(endDate1) &&
//           CommonUtils.tctCompareDates(startDate1, endDate2) < 0) ||
//         (!CommonUtils.isNullOrEmpty(endDate1) &&
...
//   // Dungpv
//   public static getParentRoute(route: ActivatedRoute, distance: number) {
//     while (distance > 0) {
//       --distance;
//       route = route.parent;
//       if (distance === 0) {
...
//   // Dungpv
//   public static transformObject(sourceObject: object, mapping: object): object {
//     // tslint:disable-next-line: forin
//     for (const property in mapping) {
//       if (sourceObject[property] && mapping[property]) {
//         sourceObject[mapping[property]] = sourceObject[property];
//         delete sourceObject[property];
//       }
//     }
//     return sourceObject;
//   }
...
//   // Dungpv
//   public static reverseTransformObject(sourceObject, mapping) {
//     // tslint:disable-next-line: forin
//     for (const property in mapping) {
//       if (sourceObject[mapping[property]] && mapping[property]) {
//         sourceObject[property] = sourceObject[mapping[property]];
//         delete sourceObject[mapping[property]];
//       }
//     }
//     return sourceObject;
//   }
...
//   //Check tuổi
//   public static yearComparison(fromYear?: any, amount?: number) {
//     var minutes = 1000 * 60;
//     var hours = minutes * 60;
//     var days = hours * 24;
//     var years = days * 365;
//     var nowYear = new Date();
//     var timeNowYear = nowYear.getTime();
//     var timeFromYear = new Date(fromYear).getTime();
//     var amountYear = (timeNowYear - timeFromYear) / years;
//     if (amountYear < amount) {
//       return true;
//     } else {
//       return false;
//     }
...
//   public static concatenationAddress(
//     province: string,
//     district?: string,
//     ward?: string,
//     village?: string,
//     address?: string
//   ) {
//     let concatenationAddress = province;
//     if (!district && ward) {
//       concatenationAddress = ward + ' - ' + concatenationAddress;
//     } else if (district && !ward) {
//       concatenationAddress = district + ' - ' + concatenationAddress;
//     } else if (district && ward) {
//       concatenationAddress = ward + ' - ' + district + ' - ' + concatenationAddress;
//     }
//     if (!village && address) {
//       concatenationAddress = address + ' - ' + concatenationAddress;
//     } else if (village && !address) {
//       concatenationAddress = village + ' - ' + concatenationAddress;
//     } else if (village && address) {
//       concatenationAddress = address + ' - ' + village + ' - ' + concatenationAddress;
//     }
//     return concatenationAddress;
//   }

  /* get real route url for lazy load component that has path empty */
  public static getRealRouteUrl(route: ActivatedRouteSnapshot): string | null {
    const routeComponent: any = route.component;
    if (!routeComponent) {
      return null;
    } else {
      const componentName = routeComponent.name;
      let url;
      if (route.url.length === 0) { url = route.parent.url.join('/'); } else { url = route.url.join('/'); }
      return url + '.' + componentName;
    }
  }

//   public static roundNumberTo(num: number, roundTo: number) {
...
//   private patientSurveySource = new BehaviorSubject("error");
//   surveyMessage = this.patientSurveySource.asObservable();

//   public setSurveyMessage(message: any) {
//     return this.patientSurveySource.next(message);
//   }

//   private immunizationDetailSource = new BehaviorSubject("error");
//   immunizationMessage = this.immunizationDetailSource.asObservable();

//   public setImmunizationMessage(message: any) {
//     return this.immunizationDetailSource.next(message);
//   }

//   private patientDetailSource = new BehaviorSubject("error");
//   patientDetailMessage = this.patientDetailSource.asObservable();

//   public setPatientDetailMessage(message: any) {
//     return this.patientDetailSource.next(message);
//   }

//   public static generateHealthFacilitiesCodeName(obj: any) {
//     if (obj.healthfacilitiesId && obj.healthfacilitiesName) {
//       obj.$healthfacilitiesCodeName = obj.healthfacilitiesId + ' - ' + obj.healthfacilitiesName;
//     }
//   }
// }
}
